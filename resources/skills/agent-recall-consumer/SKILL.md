...
The compiled draft starts with an empty `guidance_outcomes` list. Add a row only for selected guidance that actually changed, confirmed, or misled your work, and leave the list empty when nothing notable happened: unjudged selections are neutral and are still recorded as selection events. Do not write prose explaining that nothing happened. An added `unknown` row needs a comment explaining why it cannot be judged.
