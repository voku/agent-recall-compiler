...
run([$agentMapBin, 'build', '--root=.', '--paths=src,tests', '--out=' . MAP_INDEX, '--phpstan-config=phpstan.neon.dist']);
run([PHP_BINARY, $agentLoopBin, 'workflow', 'plan', TASK_ID,
    '--by', ACTOR,
...
$selectionReport = findFirstFile(RECALL_ROOT, static fn (string $path): bool =>
    basename($path) === 'selection-report.json' && basename(dirname($path)) === TASK_ID);
if ($selectionReport === null) {
    fail('context explain dogfood: selection-report.json not found');
}
$recallDir = dirname($selectionReport);
copyStrict($selectionReport, REPORT_DIR . '/selection-report.json');
copyStrict($recallDir . '/system.md', REPORT_DIR . '/system.md');
...
$system = readText($recallDir . '/system.md');
...
writeJson(REPORT_DIR . '/result.json', [
    'schema_version' => '1.0', 'task_id' => TASK_ID, 'selection_report' => $selectionReport,
    'checks' => $checks, 'passed' => !in_array(false, $checks, true),
]);
assertChecks($checks, 'context explain semantic check', 43);

run([PHP_BINARY, $agentRecallBin, 'compile',
    '--root', LEARNING_ROOT,
    '--task', 'ARC-17-TARGET',
    ...
    '--map-index', MAP_INDEX, '--map-root', '.', '--output-dir', TARGET_DIR,
...
$primary = findItem($targetItems, static fn (array $item): bool =>
    ($item['use'] ?? null) === 'implementation_candidate' && ($item['state'] ?? null) === 'verified');
$contextOnly = findItem($targetItems, static fn (array $item): bool =>
    ($item['use'] ?? null) === 'context_only_do_not_edit_from_selection_alone');
$targetChecks = [
...
writeJson(REPORT_DIR . '/targeted-result.json', [
    'schema_version' => '1.0', 'checks' => $targetChecks, 'passed' => !in_array(false, $targetChecks, true),
]);
assertChecks($targetChecks, 'targeted context explain semantic check', 44);

writeText(REPORT_DIR . '/l1.md', <<<'MARKDOWN'
## Goal
...
run([PHP_BINARY, $agentLoopBin, 'workflow', 'contract', TASK_ID,
    '--status', 'ready', '--from', REPORT_DIR . '/l1.md', '--by', ACTOR]);

$validationStarted = hrtime(true);
$composerExit = run(['composer', 'ci'], REPORT_DIR . '/composer-ci.log', true);
$validationDurationMs = intdiv(hrtime(true) - $validationStarted, 1_000_000);
fwrite(STDOUT, readText(REPORT_DIR . '/composer-ci.log'));
if ($composerExit !== 0) {
    fail('context explain dogfood: composer ci failed with exit code ' . $composerExit, $composerExit);
}

$runData = readJson(STATE_ROOT . '/runs/' . TASK_ID . '/run.json');
...
run([PHP_BINARY, $agentLoopBin, 'session', 'validation', 'record', TASK_ID,
    '--contract-revision', (string) $contractRevision,
    '--command', 'composer ci', '--status', 'passed', '--exit-code', '0',
    '--duration-ms', (string) $validationDurationMs, '--by', ACTOR]);

$reviewExit = run([PHP_BINARY, $agentLoopBin, 'review', 'blindspots', TASK_ID], REPORT_DIR . '/review-before-checkpoint.txt', true);
writeText(REPORT_DIR . '/review-before-checkpoint.exit', $reviewExit . PHP_EOL);
if ($reviewExit !== 0) {
    fail('context explain dogfood: review blindspots exited with ' . $reviewExit, 45);
}
$reviewArtifact = findFirstFile('.', static fn (string $path): bool => basename($path) === TASK_ID . '.blindspots.json');
if ($reviewArtifact === null) {
    fail('context explain dogfood: blind-spot review artifact not found', 45);
}
copyStrict($reviewArtifact, REPORT_DIR . '/blindspots.json');

run([PHP_BINARY, $agentLoopBin, 'session', 'checkpoint', TASK_ID,
    '--title', 'Review',
    '--body', 'review blindspots ' . TASK_ID . ' was generated and inspected by context-explain dogfood.']);
run([PHP_BINARY, $agentLoopBin, 'workflow', 'status', TASK_ID], REPORT_DIR . '/workflow-status.txt');
...
function run(array $command, ?string $outputPath = null, bool $allowFailure = false): int
{
    if ($outputPath !== null) {
    ...
    $process = proc_open($command, [0 => ['file', 'php://stdin', 'r'], 1 => $stdout, 2 => $stderr], $pipes);
...
{
    writeText($path, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR) . PHP_EOL);
}
...
{
    $data = json_decode(readText($path), true, 512, JSON_THROW_ON_ERROR);
    if (!is_array($data)) {
...
function copyStrict(string $source, string $destination): void
{
...
/** @param array<array-key, mixed> $items @return array<string, mixed>|null */
