...
## Verification
Run `composer ci`. Run the semantic dogfood checks against governed and target-aware Recall artifacts. Generate and inspect the agent-loop blind-spot review.

## Done When
`composer ci` passes, semantic context-explain checks pass, target-aware Recall exposes a verified implementation candidate, and the review artifact is generated and inspected without weakening the approved contract.
MARKDOWN
...
echo "[OK] context explain dogfood: governed context, contract, validation, target-aware evidence, and review were exercised"
